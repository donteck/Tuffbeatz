...
function tuff_beatz_project_audio_box($post) {
    ...
    <div class="tb-admin-field"><label for="tb_key">Key</label><input id="tb_key" name="tb_key" value="<?php echo esc_attr(get_post_meta($post->ID, '_tb_key', true)); ?>" placeholder="F# Minor"></div>
    <?php
}
...
function tuff_beatz_player_tracks() {
    ...
    while ($q->have_posts()) {
        ...
        if (!$audio) continue;
        $tracks[] = array(
            'id'=>get_the_ID(),'title'=>get_the_title(),'artist'=>get_post_meta(get_the_ID(), '_tb_artist_name', true) ?: 'TUFF BEATZ','audio'=>esc_url_raw($audio),
            'cover'=>get_the_post_thumbnail_url(get_the_ID(), 'medium') ?: get_template_directory_uri() . '/assets/images/tuff-beatz-logo.png','url'=>get_permalink(),
            'stream'=>get_post_meta(get_the_ID(), '_tb_stream_url', true),'buy'=>get_post_meta(get_the_ID(), '_tb_buy_url', true),'genre'=>get_post_meta(get_the_ID(), '_tb_genre', true),
            'bpm'=>get_post_meta(get_the_ID(), '_tb_bpm', true),'key'=>get_post_meta(get_the_ID(), '_tb_key', true),
        );
    }
    wp_reset_postdata();
    return $tracks;
...
function tuff_beatz_player_data() {
    wp_localize_script('tuff-beatz-main', 'TUFF_BEATZ_PLAYER', array('tracks'=>tuff_beatz_player_tracks(),'logo'=>get_template_directory_uri() . '/assets/images/tuff-beatz-logo.png'));
}
