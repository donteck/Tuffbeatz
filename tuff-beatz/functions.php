...
function tuff_beatz_project_audio_box($post) {$audio = get_post_meta($post->ID, '_tb_audio_url', true);$artist = get_post_meta($post->ID, '_tb_artist_name', true);$stream = get_post_meta($post->ID, '_tb_stream_url', true);$buy = get_post_meta($post->ID, '_tb_buy_url', true);wp_nonce_field('tuff_beatz_save_project_audio', 'tuff_beatz_project_audio_nonce');?><style>.tb-admin-field{margin:0 0 16px}.tb-admin-field label{display:block;font-weight:700;margin-bottom:5px}.tb-admin-field input{width:100%}</style><div class="tb-admin-field"><label>Artist name</label><input name="tb_artist_name" value="<?php echo esc_attr($artist); ?>"></div><div class="tb-admin-field"><label>Audio preview / MP3 URL</label><input name="tb_audio_url" value="<?php echo esc_attr($audio); ?>"></div><div class="tb-admin-field"><label>Listen / Smart Link</label><input name="tb_stream_url" value="<?php echo esc_attr($stream); ?>"></div><div class="tb-admin-field"><label>Buy / License Link</label><input name="tb_buy_url" value="<?php echo esc_attr($buy); ?>"></div><div class="tb-admin-field"><label>Genre</label><input name="tb_genre" value="<?php echo esc_attr(get_post_meta($post->ID, '_tb_genre', true)); ?>"></div><div class="tb-admin-field"><label>BPM</label><input name="tb_bpm" value="<?php echo esc_attr(get_post_meta($post->ID, '_tb_bpm', true)); ?>"></div><div class="tb-admin-field"><label>Key</label><input name="tb_key" value="<?php echo esc_attr(get_post_meta($post->ID, '_tb_key', true)); ?>"></div><?php }
function tuff_beatz_save_project_audio($post_id) {if (!isset($_POST['tuff_beatz_project_audio_nonce']) || !wp_verify_nonce($_POST['tuff_beatz_project_audio_nonce'], 'tuff_beatz_save_project_audio')) return;if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) return;if (!current_user_can('edit_post', $post_id)) return;$map = array('tb_audio_url'=>'_tb_audio_url','tb_artist_name'=>'_tb_artist_name','tb_stream_url'=>'_tb_stream_url','tb_buy_url'=>'_tb_buy_url','tb_genre'=>'_tb_genre','tb_bpm'=>'_tb_bpm','tb_key'=>'_tb_key');foreach ($map as $field => $meta) if (isset($_POST[$field])) {$value = in_array($field, array('tb_artist_name','tb_genre','tb_bpm','tb_key'), true) ? sanitize_text_field($_POST[$field]) : esc_url_raw($_POST[$field]);update_post_meta($post_id, $meta, $value);}}
