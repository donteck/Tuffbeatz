...
function tuff_beatz_portal_assets() {
    if (is_page('start-a-project')) {
        ...
        wp_enqueue_style('tuff-beatz-project-portal', get_template_directory_uri() . '/assets/css/project-portal.css', array('tuff-beatz-main'), file_exists($path) ? filemtime($path) : wp_get_theme()->get('Version'));
    }
}
...
function tuff_beatz_register_portal_account() {
    ...
    $base = sanitize_user(strtolower(strtok($email, '@')), true); if (!$base) $base = 'artist';
    ...
    if (is_wp_error($user_id)) { wp_safe_redirect(add_query_arg('portal_error', 'registration_failed', $redirect)); exit; }
    wp_set_current_user($user_id); wp_set_auth_cookie($user_id, true); wp_safe_redirect(add_query_arg('welcome', '1', $redirect)); exit;
}
...
function tuff_beatz_submit_project_request() {
    ...
    if (!$artist_name || !$project_title || !$service || !$details) { wp_safe_redirect(add_query_arg('portal_error', 'required_fields', $redirect)); exit; }
    $request_id = wp_insert_post(array('post_type'=>'tb_request','post_status'=>'publish','post_title'=>sprintf('%s — %s',$artist_name,$project_title),'post_author'=>$user->ID));
    if (is_wp_error($request_id) || !$request_id) { wp_safe_redirect(add_query_arg('portal_error', 'submit_failed', $redirect)); exit; }
    $meta = array('_tb_request_artist'=>$artist_name,'_tb_request_service'=>$service,'_tb_request_project_type'=>$project_type,'_tb_request_genre'=>$genre,'_tb_request_budget'=>$budget,'_tb_request_target_date'=>$target_date,'_tb_request_reference_url'=>$reference_url,'_tb_request_details'=>$details,'_tb_request_status'=>'new','_tb_request_email'=>$user->user_email);
    ...
    if (!empty($_FILES['project_file']['name'])) {
        ...
        $allowed = array('mp3','wav','m4a','zip','pdf'); $ext = strtolower(pathinfo($_FILES['project_file']['name'], PATHINFO_EXTENSION));
        if (in_array($ext,$allowed,true)) { $attachment_id = media_handle_upload('project_file',$request_id); if (!is_wp_error($attachment_id)) update_post_meta($request_id,'_tb_request_attachment',(int)$attachment_id); }
    }
    $admin_email = tuff_beatz_get('contact_email', get_option('admin_email'));
    wp_mail($admin_email, sprintf('[TUFF BEATZ] New Project Request — %s',$project_title), "A new project request has been submitted.\n\nArtist/Client: {$artist_name}\nEmail: {$user->user_email}\nService: {$service}\nProject: {$project_title}\nBudget: {$budget}\n\nReview it in WordPress → Project Requests.");
    wp_safe_redirect(add_query_arg(array('submitted'=>'1','request'=>$request_id),$redirect)); exit;
}
...
function tuff_beatz_request_admin_box($post) {
    ...
    $fields = array('Artist / Client'=>get_post_meta($post->ID,'_tb_request_artist',true),'Email'=>get_post_meta($post->ID,'_tb_request_email',true),'Service'=>get_post_meta($post->ID,'_tb_request_service',true),'Project Type'=>get_post_meta($post->ID,'_tb_request_project_type',true),'Genre'=>get_post_meta($post->ID,'_tb_request_genre',true),'Budget'=>get_post_meta($post->ID,'_tb_request_budget',true),'Target Date'=>get_post_meta($post->ID,'_tb_request_target_date',true));
    echo '<div style="display:grid;gap:10px">'; foreach($fields as $label=>$value) echo '<div><strong>'.esc_html($label).':</strong> '.esc_html($value ?: '—').'</div>';
    $ref=get_post_meta($post->ID,'_tb_request_reference_url',true); if($ref) echo '<div><strong>Reference:</strong> <a href="'.esc_url($ref).'" target="_blank" rel="noopener">'.esc_html($ref).'</a></div>';
    $attachment=(int)get_post_meta($post->ID,'_tb_request_attachment',true); if($attachment) echo '<div><strong>Uploaded file:</strong> <a href="'.esc_url(wp_get_attachment_url($attachment)).'" target="_blank" rel="noopener">Open file</a></div>';
    ...
    echo '<label><strong>Status</strong><br><select name="tb_request_status">'; $current=get_post_meta($post->ID,'_tb_request_status',true) ?: 'new';
    foreach(array('new'=>'New','reviewing'=>'Reviewing','approved'=>'Approved','in-progress'=>'In Progress','completed'=>'Completed','declined'=>'Declined') as $value=>$label) echo '<option value="'.esc_attr($value).'" '.selected($current,$value,false).'>'.esc_html($label).'</option>';
...
function tuff_beatz_save_request_admin($post_id){ if(!isset($_POST['tb_request_admin_nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['tb_request_admin_nonce'])),'tb_request_admin_save')) return; if(!current_user_can('edit_post',$post_id)) return; if(isset($_POST['tb_request_status'])){$status=sanitize_key(wp_unslash($_POST['tb_request_status']));$allowed=array('new','reviewing','approved','in-progress','completed','declined');if(in_array($status,$allowed,true)) update_post_meta($post_id,'_tb_request_status',$status);} }
add_action('save_post_tb_request','tuff_beatz_save_request_admin');
function tuff_beatz_request_status_label($status){$labels=array('new'=>'New','reviewing'=>'Reviewing','approved'=>'Approved','in-progress'=>'In Progress','completed'=>'Completed','declined'=>'Declined');return $labels[$status] ?? 'New';}
