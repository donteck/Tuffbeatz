...
function tuff_beatz_studio_os_dock(){if(!tuff_beatz_is_studio_os_page()||!is_user_logged_in())return;$producer=tuff_beatz_is_producer_user();$uri=trim(parse_url($_SERVER['REQUEST_URI']??'/',PHP_URL_PATH),'/');echo '<nav class="tb-os-dock" aria-label="TUFF BEATZ Studio OS"><span>STUDIO OS</span>';
 if($producer){$items=array('producer-portal'=>'Command','producer-crm'=>'CRM','start-a-project'=>'Intake');}else{$items=array('client-portal'=>'My Projects','start-a-project'=>'New Project');}
 foreach($items as $slug=>$label){$active=$uri===$slug?' is-active':'';echo '<a class="'.esc_attr(trim($active)).'" href="'.esc_url(home_url('/'.$slug.'/')).'">'.esc_html($label).'</a>';}
 if(is_page('project-dashboard'))echo '<a class="is-active" href="#overview">Workspace</a>';
 echo '<a href="'.esc_url(wp_logout_url(home_url('/'))).'">Sign Out</a></nav>';}
add_action('wp_footer','tuff_beatz_studio_os_dock',60);
