...
/** TUFF BEATZ Production Dashboard V1 */
function tuff_beatz_dashboard_page_setup(){
    $page=get_page_by_path('project-dashboard');
    if(!$page){
        $id=wp_insert_post(array('post_title'=>'Project Dashboard','post_name'=>'project-dashboard','post_status'=>'publish','post_type'=>'page','post_content'=>''));
        if(!is_wp_error($id)&&$id) update_post_meta($id,'_wp_page_template','page-project-dashboard.php');
    } else {
        update_post_meta($page->ID,'_wp_page_template','page-project-dashboard.php');
    }
}
add_action('init','tuff_beatz_dashboard_page_setup',31);

function tuff_beatz_dashboard_assets(){
    if(is_page('project-dashboard')){
...
function tuff_beatz_user_can_view_request($request_id,$user_id=0){
    ...
    if(!$request_id||!$user_id)return false;
    if(current_user_can('edit_post',$request_id))return true;
    $post=get_post($request_id);
    return $post&&$post->post_type==='tb_request'&&(int)$post->post_author===(int)$user_id;
}

function tuff_beatz_project_dashboard_url($request_id){
    return add_query_arg('project',(int)$request_id,home_url('/project-dashboard/'));
}

function tuff_beatz_dashboard_messages($request_id){
    $messages=get_post_meta((int)$request_id,'_tb_project_messages',true);
    return is_array($messages)?$messages:array();
}

function tuff_beatz_submit_project_message(){
    $request_id=(int)($_POST['request_id']??0);
    $redirect=tuff_beatz_project_dashboard_url($request_id);
    ...
    if(!$body){wp_safe_redirect(add_query_arg('dash_error','message',$redirect));exit;}
    $u=wp_get_current_user();$messages=tuff_beatz_dashboard_messages($request_id);
    $messages[]=array('user_id'=>$u->ID,'name'=>$u->display_name,'role'=>current_user_can('edit_post',$request_id)?'TUFF BEATZ':'Client','body'=>$body,'time'=>current_time('mysql'));
    update_post_meta($request_id,'_tb_project_messages',$messages);
...
function tuff_beatz_upload_project_dashboard_files(){
    ...
    $new=tuff_beatz_handle_project_uploads($request_id);$existing=get_post_meta($request_id,'_tb_request_attachments',true);if(!is_array($existing))$existing=array();
    if($new)update_post_meta($request_id,'_tb_request_attachments',array_values(array_unique(array_merge($existing,$new))));
...
function tuff_beatz_dashboard_admin_box($post){
    ...
    echo '<p><label><strong>Final Delivery Links</strong></label><br><textarea style="width:100%;min-height:90px" name="tb_request_final_links" placeholder="One secure delivery/download link per line">'.esc_textarea($final).'</textarea></p>';
    echo '<hr><p><strong>Project Messages:</strong> '.count($messages).'</p>';
}
